...
const UPLOAD_DIR = "/app/uploads" // ⚠️ IMPORTANT : plus dans public
const ALLOWED = [
  "image/jpeg",
  "image/png",
  "image/webp",
  ...
  const images = await Promise.all(
    files
      ...
      .map(async (f) => {
        ...
        return {
          key: f,
          url: `/api/uploads/${f}`, // ⚠️ on passe par l'API maintenant
          size: s.size,
...
export async function POST(req: Request) {
  ...
  return Response.json({
    key: filename,
    url: `/api/uploads/${filename}`, // ⚠️ important
  })
}